...
pub fn get_exchange_booth_pda(
    ...
    if xbooth_pda != *exchange_booth_account.key {
        ...
        return Err(ExchangeBoothError::InvalidVaultAccount.into());
    }
...
pub fn get_vault_pda(
    ...
    Ok((vault_pda, vault_bump_seed))
}

// convert amount to lamports by decimals
pub fn amount_to_lamports(mint: &AccountInfo, amount: u64) -> Result<u64, ProgramError> {
    ...
    let lamports = (amount as f64 * f64::powf(10., mint_decimals.into())) as u64;
    Ok(lamports)
}
...
pub fn check_stored_owner(
    ...
    let exchange_booth_account_data = ExchangeBooth::try_from_slice(&data).unwrap();
    if authority.key != &exchange_booth_account_data.admin {
        ...
        return Err(ExchangeBoothError::InvalidOwner.into());
    }
    Ok(())
}